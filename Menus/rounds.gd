...
func _ready() -> void:
	blue_score.text = "Blue: " + str(Globals.roundData.blue_score)
	red_score.text = "Red: " + str(Globals.roundData.red_score)
	
func _process(delta: float) -> void:
	run_timer += delta
	if run_timer > run_delay && (Input.is_joy_button_pressed(0, JOY_BUTTON_X) || Input.is_joy_button_pressed(1, JOY_BUTTON_X)):
		if Globals.roundData.blue_score >= 5 || Globals.roundData.red_score >= 5:
			Globals.roundData.blue_score = 0
			Globals.roundData.red_score = 0
			get_tree().change_scene_to_file("res://Menus/menu.tscn")
		else:
			get_tree().change_scene_to_file(Globals.default_game_scene)
